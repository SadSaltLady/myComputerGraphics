...
Spectrum Env_Map::sample_direction(Vec3 dir) const {
    ...
    float phi = (atan2(dir.z, dir.x) + PI_F); //not sure how to calculate

    //DEBUG
    assert(theta >= 0.0f && theta <= PI_F);
    assert(phi >= 0.0f && phi <= 2.0f * PI_F); 
    //lookup in texture
    float XX = (phi / (2.0f * PI_F) * w);
    float YY = (theta  * h/ PI_F);

    //find base index
    size_t x = (size_t)floor(XX + 0.5f) % w;
    size_t y = (size_t)floor(YY + 0.5f) % h;
